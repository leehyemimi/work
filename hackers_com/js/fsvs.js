		...
		var app = {
			...
			addPagination : function() {
				...
				$('li', pagination).click( function(e){
					...
					//alert(nm);
					$("body").removeClass( 'w_f' );
				});
				
				// 동영상스크립트 추가
				$('gnb_mn li').click(function(e){
					ignoreHashChange = true;
					app.slideToIndex( $(this).index(), e );
					$('.active', pagination).removeClass( 'active' );
					$(this).addClass( 'active' );
			...
			slideToIndex : function( index, e ) {
				...
				$(".gnb_mn li").eq(index).addClass('active');
				$(".wrap").removeClass( 'w_f' );
				
				// dunkin
				if (index == 1) //  slide-2일 경우
				{
					if ( $('.active-slide-2').find('.slidesjs-pagination-item0').find('a').hasClass('active') )
					{
						//jwplayer("myElement").play(); // error on ie8 // slide-2 Hackers MOVIE 일 경우만
						$('.mn01').addClass('on');
						$('.mn02').removeClass('on');
						$('.mn03').removeClass('on');
